  ...
  <div class="class-section" v-if="total > 0">
    <div class="class-head">
      <div class="class-left">
        <span class="class-title">我的课程</span>
        <span class="class-number">共有 {{ total }} 门课程</span>
      </div>
      <div>
    ...
    <div class="class-content">
      <div v-for="item of courses" :key="item.id">
        <course-card :list="item" @itemClick="goAcademyCourseDetail(item.id)" />
      </div>
      <pagination
...
export default {
  components: { SearchBar, CourseCard, Pagination, TheEmpty },
  name: "MyCourses",
  props: {
    loading: {
      type: Boolean,
